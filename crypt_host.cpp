...
void crypt_HOST::myCrypt(int encdec, const char * text) {

  const char * pass = passw.toStdString().c_str();
  const char * salt = saltw.toStdString().c_str();
  ...
  // используем алгоритм шифрования - GCRY_CIPHER_GOST28147
  ...
  switch (encdec) {
    case ENCR:
      printf("Ecnrypted text in file res.txt");
      for (i = 0; i<textLength; i++) fprintf(fp, "%02X", (unsigned char)outBuffer[i]);
      printf("\n");
      break;
    case DECR:
